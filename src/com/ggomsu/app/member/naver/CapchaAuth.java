...
public class CapchaAuth implements Action{
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if(capchaKey != null && value != null) {
        	String apiURL = NaverInfo.CAPCHA_AUTH_URL + "?code=1" + "&key=" + capchaKey + "&value=" + value;
        	...
        	// 임시 세션 발급
        	if(result) {
        		MemberVO member = dao.getMemberInfo("noname@ggomsu.com");
        		
    			String articleForward = (String) session.getAttribute("articleForward");
    			
    			session.setAttribute("statusValue", member.getStatusValue());
				session.setAttribute("email", member.getEmail());
				session.setAttribute("nickname", member.getNickname());
				session.setAttribute("profileImageUrl", null);
				
				// 이전에 보던 페이지가 있는가?
				if(articleForward != null) {
