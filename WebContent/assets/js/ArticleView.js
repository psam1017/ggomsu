...
const articleLikeCountContainer = document.getElementById("articleLikeCountContainer");
        ...
        if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
        	...
        	if(json.status == "do"){
        		articleLikePath.forEach(element => {
        			...
        			articleLikeCountContainer.innerText = json.articleLikeCount;
        		});
        	}
...
function commentLike(commentIndex, commentLikePath, commentLikeCountContainer){
    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
        	...
        	if(json.status == "do"){
        		...
        		commentLikeCountContainer.innerText = json.commentLikeCount;
        	}
        	else if(json.status == "cancel"){
...
function showCommentList(commentList){
	...
	let commentLikeCountContainer = document.querySelectorAll(".commentLikeCountContainer");
	
	if(commentLikeButton != null){
		commentLikeButton.forEach((element, index) => {
			element.addEventListener("click", function(e){
				e.preventDefault();
				commentLike(element.dataset.value, commentLikePath[index], commentLikeCountContainer[index]);
			});
		});
	}
}
