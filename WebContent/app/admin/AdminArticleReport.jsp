	    ...
	    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
	        <div class="table-responsive" style="width:100%; height:40vh; overflow:auto">
	          <table class="table">
	            ...
	            <tbody>
	            <c:choose>
	                <c:when test="${articleReportList != null and fn:length(articleReportList) > 0}">
	                    <c:forEach var="articleReport" items="${articleReportList}">
	                    	<c:if test="${articleReport.getArticleDeleteCheck() == 0}">
		                        <tr class="clickable-tr">
		                            <th scope="row"><c:out value="${articleReport.getArticleIndex()}"/></th>
		                            <td><c:out value="${articleReport.getNickname()}"/></td>
				                     <td><c:out value="${articleReport.getArticleReportReason()}"/></td>
				                     <td>
				                      	<a class="btn btn-primary" href="${pageContext.request.contextPath}/board/article-view-detail-ok?articleIndex=${articleReport.getArticleIndex()}">
				                            	상세보기
				                        </a>
		                      		</td>
		                   		</tr>
